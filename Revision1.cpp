...
for(int cr=45;cr<=131;cr++)
    ...
    if(cr>52 && cr<86){gotoxy(1,86-cr);    printf("                              HAMMAD RIAZ (SP18-BCS-127)                     ");}
...
// they are checked by values and the conditions depending on those values
void shot(int Arr[25][4],int X[5],int Y[5],int &wq,int &er,int pp[5],int qq[5],int &f1,int &tra,int &luf)
{
   for(int j=0;j<5;j++)
   {
...
//this function is used to determine which is the greatest value and its purpose is to
  ...
  for(int i=0;i<25;i++)
  {
   ...
   break;
    ...
    if(w1!=w2)
    {
      ...
      printf("/");
      s=d[c[t]-1][0];  //compueters choice is this
      for(int j=w;j<b;j++)
        {
            c[j]=c[j+1];
        }
